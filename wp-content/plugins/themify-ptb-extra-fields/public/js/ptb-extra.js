var PTB_MapInit = function() {
    ...
    function Initialize($map, $options) {
            if($options.display && $options.display === 'text'){
		
                    var opt, geocoder = new google.maps.Geocoder;
        ...
        function PTB_Map(){
            ...
            if ($('.ptb_extra_map').length > 0) {
                if (typeof google !== 'object' || typeof google.maps !== 'object' || typeof google.maps.places === 'undefined') {
                    ...
                    loadScript('//maps.googleapis.com/maps/api/js?v=3.exp&libraries=places&callback=PTB_MapInit&language=' + ptb_extra.lng+'&key='+ptb_extra.map_key);
                } else {
                    PTB_MapInit();
                }
            }
