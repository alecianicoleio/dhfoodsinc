...
class PTB_CMB_Map extends PTB_Extra_Base {
    ...
    public function action_them_themplate($id, $type, $args, $data = array(), array $languages = array()) {
        ...
        $display = array('map' => __('Map', 'ptb_extra'), 'text' => __('Address Text', 'ptb_extra') );
        ?>
		<div class="ptb_back_active_module_row">
            <div class="ptb_back_active_module_label">
                <label for="ptb_<?php echo $id ?>[displayAs]"><?php _e('Display As', 'ptb_extra') ?></label>
            </div>
    ...
    public function render_post_type_meta($post, $meta_key, $args) {
        ...
        <table class="ptb_extra_map_info" width="100%">
            <?php if(!PTB_Extra_Base::get_google_map_key()):?>
            <tr>
                <td colspan="2">
                    <span class="ptb_extra_map_missing"><?php _e('Error: Google Maps API key is missing','ptb_extra') ?></span>
                    <a target="_blank"  href="<?php echo add_query_arg('page','ptb-map',admin_url('admin.php'))?>"><?php _e('Add Map Key','ptb_extra') ?></a>
                </td>
            </tr>
            <?php endif;?>
            <tr>
            ...
            <tr>
                <td><label for="ptb_extra_<?php echo $meta_key ?>_lat"><?php _e('Lat,Long', 'ptb_extra') ?></label></td>
    ...
    public function ptb_submission_form($post_type, array $args, array $module, $post, $lang, $languages) {
        wp_enqueue_script(self::$plugin_name . '-submission-map', PTB_Utils::enque_min(plugin_dir_url(dirname(__FILE__)) . 'public/submission/js/map.js'), array('ptb-submission'), self::$version, true);
        if (isset($post->ID)) {
    ...
    public function ptb_submission_save(array $m, $key, array $post_data, $post_id, $lng) {
     ...
     public function search_map_template($post_type,$id,$args,$module,$value,$label,$lang,$languages){
        ...
        wp_enqueue_script(self::$plugin_name . '-search-map',PTB_Utils::enque_min( $pluginurl . 'public/search/js/map.js'), array('ptb-search'), self::$version, true);
