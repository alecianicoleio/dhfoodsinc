...
class PTB_Extra_Base extends PTB_CMB_Base {
    ...
    public static function Init($version) {
        ...
        if (is_admin()) {
            ...
            add_filter('ptb_min_filess',array(__CLASS__,'add_min_files'),10,1);
            add_action('wp_enqueue_scripts', array(__CLASS__, 'public_enqueue_scripts'));
            add_filter('ptb_filter_cpt_category_archives', array(__CLASS__, 'filter_cpt_category_archives'), 10, 2);
        }
    ...
    public static function enqueue_scripts() {
        ...
        if ($screen->id !== 'customize') {
            ...
            if (in_array($screen->id, $screens,true)) {
                ...
                wp_enqueue_script(self::$plugin_name . '-slider', PTB_Utils::enque_min($pluginurl . 'admin/js/ptb-extra-slider.js'), array('ptb'), self::$version, false);
                wp_enqueue_script(self::$plugin_name . '-gallery', PTB_Utils::enque_min($pluginurl . 'admin/js/ptb-extra-gallery.js'), array('ptb'), self::$version, false);
                wp_enqueue_script(self::$plugin_name . '-rating', PTB_Utils::enque_min($pluginurl . 'admin/js/ptb-extra-rating.js'), array('ptb'), self::$version, false);
                wp_enqueue_script(self::$plugin_name . '-progress-bar', PTB_Utils::enque_min($pluginurl . 'admin/js/ptb-extra-progress-bar.js'), array('ptb'), self::$version, false);
                ...
                wp_enqueue_script(self::$plugin_name . '-telephone', PTB_Utils::enque_min($pluginurl . 'admin/js/ptb-extra-telephone.js'), array('ptb'), self::$version, false);
    ...
    public static function public_enqueue_scripts() {
        ...
        wp_enqueue_style(self::$plugin_name, PTB_Utils::enque_min($pluginurl . 'public/css/ptb-extra.css'), array(), self::$version, 'all');
        wp_enqueue_style(self::$plugin_name . '-submission', PTB_Utils::enque_min($pluginurl . 'public/submission/css/ptb-extra-submission.css'), array('ptb-submission'), self::$version, 'all');
        $translation_ = array(
            'lng' => PTB_Utils::get_current_language_code(),
        ...
        wp_register_script(self::$plugin_name, PTB_Utils::enque_min($pluginurl . 'public/js/ptb-extra.js'), array('ptb'), self::$version, true);
    ...
    public static function check_order_field($args) { 
        if (PTB_Public::$shortcode && isset($args['meta_key']) && preg_match('/^ptb_.+?_(start|end)$/', $args['meta_key'])) {
            $cmb = PTB::get_option()->get_cpt_cmb_options($args['post_type']);
            $meta_key = str_replace(array('_start','_end','ptb_'), array('','',''), $args['meta_key']); 
            if(isset($cmb[$meta_key]) && $cmb[$meta_key]['type']=== 'event_date'){
    ...
    public static function order_by_date($orderby, $wp_query) {
        if (self::$sort_filed && ( PTB_Public::$shortcode || $wp_query->get( 'order_by_date' ) ) ) {
    ...
    public static function filter_cpt_category_archives( $query, $archive ) {
        if( $archive && $archive->get_archive() ) {
                ...
                if( ! isset(PTB_Form_PTT_Archive::$sortfields[$archive['ptb_ptt_orderby_post']]) && isset( $query->query['post_type'] ) ) {
                        ...
                        if(! isset( $options[$order_by_key] ) && ! $query->get( 'meta_key' ) && preg_match( '/_(?:start|end)$/', $order_by_key, $match ) && ! empty( $match[0] ) ) {
                                ...
                                self::$sort_filed = strpos($match[0], '_start') !== FALSE ? 'start' : 'end';
    ...
    public static function add_templates($dir){
        $dir[1] = trailingslashit(plugin_dir_path(__DIR__)).'templates';
        return $dir;
    }
    
    public static function add_min_files($files){
        $files['css']['jquery.bxslider'] =PTB_Utils::enque_min(plugin_dir_url(__DIR__) . 'public/css/jquery.bxslider.css',true);
        return $files;
    }
